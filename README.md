...
* **Links**: `[text](https://example.com)` converted to live hyperlinks
...
The modules use late binding: `CreateObject("VBScript.RegExp")`. Because of this, the regex library does not need to be manually enabled under Tools > References.

*   Tables are now supported.
*   Task lists are now supported.
*   Regex dependency is not required because the code uses late binding with `CreateObject("VBScript.RegExp")`.
*   Macro names should match the modules:
    *   `PasteMarkdownToWord`
    *   `PasteMarkdownToEmail`
*   Removed emojis.
*   Removed em dashes.
*   Fixed typo: `bginning` to `beginning`.
